...
var transporter = nodemailer.createTransport({
 ...
 app.post('/additem', (req, res)=>{
    if(req.session.userId){
       ...
       if(req.body.content){
        req.body.id = "twt" + String(Date.now()) + String(req.session.userId) + String(Math.floor(Math.random() *(28000 - 15) + 15));
 ...
 app.post('/search', (req, res)=>{
    ...
    //         res.json({status:"OK", items:response.hits.hits});
    //       }
    //   });
    });
  ...
  db.Tweet.find({id: req.params.id}).lean().then((data)=>{
    ...
    if (error0) {
      ...
      connection.createChannel(function(error1, channel) {
        ...
        let file = {media:data[0].media, id:req.params.id, username:req.session.username}
        channel.sendToQueue(queue, Buffer.from(JSON.stringify(file)), {
            persistent: true
        });
    });
